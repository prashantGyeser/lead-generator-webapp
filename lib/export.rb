class Export
  ...
  def non_leads_to_table
    NonLead.where("created_at >= :date", date: "2014-12-01").where.not(keyword_id: nil).find_each do |non_lead|
      begin

        keyword = Keyword.find(non_lead.keyword_id)
        lead_stream = LeadStream.find(keyword.lead_stream_id)
        user = User.find(lead_stream.user_id)
        sample_category = SampleCategory.find(user.sample_category_id)

        training_data_export = TrainingDataExport.new(tweet_id: non_lead.tweet_id, tweet_body: non_lead.tweet_body, training_data_type: "non_lead", category: sample_category.name, keyword_term: keyword.term )
